...
export function createLiteWorkbookPdf(args: { address: string; rows: WorkbookRow[] }): Buffer {
  const contentLines = [
    ...
    args.address,
    "",
    "Rank | Business | Category | Property Type | Type | Location | Fit | Move | Summary | Rationale",
    ...args.rows.flatMap((row) => [
      `${row.priority_rank}. ${row.business_name} | ${row.category} | ${row.property_type} | ${row.type} | ${row.city}, ${row.state} | ${row.tenant_fit_score_100}/100 | ${row.move_probability_1_10}/10`,
