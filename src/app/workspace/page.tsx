  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // no-op for non-blocking usage panel
      }
    })();
  ...
  return (
    <main className={`app-shell ${sidebarCollapsed ? "collapsed" : ""}`}>
      <aside className="primary-sidebar">
        <div className="sidebar-top">
          {!sidebarCollapsed ? (
            <div style={{ display: "grid" }}>
              <span className="sidebar-brand">Timpani</span>
              <span style={{ fontSize: 11, color: "#64748b" }}>{usageSnapshot?.planName ?? "Free"} plan</span>
            </div>
          ) : (
            <span />
          )}
        ...
        {!sidebarCollapsed && usageSnapshot ? (
          <div style={{ padding: "0 10px 12px", display: "grid", gap: 6 }}>
            <small style={{ color: "#64748b" }}>
              Listings {usageSnapshot.usage.LISTINGS}/{usageSnapshot.limits.LISTINGS}
            </small>
            <small style={{ color: "#64748b" }}>
              Contacts {usageSnapshot.usage.CONTACTS}/{usageSnapshot.limits.CONTACTS}
            </small>
            <small style={{ color: "#64748b" }}>
              Workbooks {usageSnapshot.usage.WORKBOOKS}/{usageSnapshot.limits.WORKBOOKS}
            </small>
            <a href="/pricing" className="btn secondary" style={{ textAlign: "center" }}>
              Upgrade
            </a>
          </div>
        ) : null}
