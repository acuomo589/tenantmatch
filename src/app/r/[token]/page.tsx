...
export default async function LitePublicLinkPage({
  ...
  const paid = canDownloadLiteLink(link);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.12),_transparent_45%),linear-gradient(180deg,_#f8fafc,_#eef2ff_55%,_#f8fafc)] px-4 py-10">
      <LiteOpenedBeacon token={token} />
      <div className="mx-auto grid max-w-5xl gap-6">
        <section className="rounded-[2rem] border bg-white/95 px-6 py-8 shadow-sm backdrop-blur md:px-8">
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
            <div className="grid gap-3">
              <Badge variant="secondary">TenantMatch</Badge>
              <div className="grid gap-2">
                <h1 className="text-3xl font-semibold tracking-tight md:text-5xl">{link.workbook.displayAddress}</h1>
            ...
            <Card className="min-w-[18rem] border-sky-100 bg-sky-50/70">
              <CardContent className="grid gap-4 p-5">
                <div className="grid gap-1">
                  ...
                  <div className="text-3xl font-semibold text-sky-950">{formatCurrency(link.priceCents, link.currency)}</div>
                </div>

                {paid ? (
                  <div className="grid gap-2">
                    <Button asChild>
                      <Link href={`/r/${token}/download/csv`}>Download CSV</Link>
                    </Button>
                    <Button asChild variant="outline">
                      <Link href={`/r/${token}/download/pdf`}>Download PDF</Link>
                    </Button>
                  </div>
                ) : (
                  <LiteCheckoutButton token={token} />
                )}
        ...
        <Card data-surface>
          <CardContent className="grid gap-5 p-6">
            <div className="flex items-center justify-between gap-3">
              <div className="grid gap-1">
                <h2 className="text-lg font-semibold tracking-tight">{paid ? "Full workbook" : "Preview row"}</h2>
                {paid ? <p className="text-sm text-muted-foreground">All 25 workbook rows are unlocked.</p> : null}
              </div>
              <Badge variant="outline">{paid ? "Paid" : "Preview"}</Badge>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/45">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Summary</th>
                  </tr>
                </thead>
                <tbody>
                  {(paid ? link.workbook.workbookRowsJson : previewRows).map((row) => (
                    <tr key={`${row.priority_rank}-${row.business_name}`} className="border-t">
                      ...
                      <td className="px-4 py-4 text-muted-foreground">{row.fit_summary}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
